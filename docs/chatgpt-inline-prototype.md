...
From another terminal, run `npm run smoke:mcp` to initialize the MCP endpoint, check the two tools and their results, and read `ui://streamlion/workspace-v1.html`. After deployment, repeat with `npm run smoke:mcp -- https://streamlion.transcendencemedia.com/mcp`. Then preview the UI in a ChatGPT developer-mode connection.
...
4. Test the embedded PWA's Google sign-in, workbook selection, project editing, and return to chat on desktop and phone. Browser policies or ChatGPT review may block nested Google flows. The card retains an “Open in browser” route for that case.
